...
int main(int argc, char** argv){
	...
	if (argc < 4 || opt_help == "-h" || opt_help == "--h" || opt_help == "--help" || opt_help == "help" ){
		cout << "Program to compute stats for d x d matrix. " << endl;
		cout << "usage: statmatrix [dimension] [interval|bin] [in_filepath] [out_filepath]" << endl;
		return 0;
	}
	...
	for (int i = 0; i < d; ++i)
		for (int j = 0; j < d; ++j)
			diamA[i][j] = (A[i][j].rightBound() - A[i][j].leftBound());

	double maxDiam = diamA[0][0];
	double minDiam = diamA[0][0];
	double avgDiam = 0.;
	for (int i = 0; i < d; ++i){
		for (int j = 0; j < d; ++j){
			avgDiam += diamA[i][j];
			if (minDiam > diamA[i][j]) minDiam = diamA[i][j];
			if (maxDiam < diamA[i][j]) maxDiam = diamA[i][j];
		}
	}
	avgDiam /= double(d * d);
	...
	cout << "# Min diam: " << minDiam << endl;
	cout << "# Avg diam: " << avgDiam << endl;
	cout << "# END STATS #" << endl;
