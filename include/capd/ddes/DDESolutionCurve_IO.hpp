...
#ifndef _CAPD_DDES_DDESOLUTIONCURVE_IO_HPP_
#define _CAPD_DDES_DDESOLUTIONCURVE_IO_HPP_
...
std::string DDESolutionCurve<SetSpec>::show() const {
	std::ostringstream oss;
	oss << "DDESolutionCurve in dimension " << dimension() << " over time interval: " << pastTime() << " to " << currentTime() << "\n";
	for (auto it = begin(); it != end(); ){
		oss << (*it)->show();
		RealType t0 = (*it)->t0(); ++it; RealType t1 = (it == end() ? this->currentTime() : (*it)->t0());
		oss << " for t \\in " << capd::ddes::showEnclosedInterval(t0, t1) << "\n";
	}
	oss << m_valueAtCurrent.hull() << " for t == " << this->currentTime();
	return oss.str();
}

template<typename SetSpec>
void DDESolutionCurve<SetSpec>::writeTo(std::ostream& out) const {
...
#endif /* _CAPD_DDES_DDESOLUTIONCURVE_IO_HPP_ */
